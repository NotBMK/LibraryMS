...
<%
  ...
  boolean found = UserDao.getUserInfoById(targetUser, targetUserId);
...
<div class="container">
  ...
  <form action="${pageContext.request.contextPath}/admin/adminEditUserInfo" method="post">
    <input type="hidden" name="userId" value="<%= targetUser.getId() %>">

    <div class="form-group">
      <label for="username">用户名</label>
      <input type="text" class="form-control" id="username" name="username" value="<%= targetUser.getName() %>">
    </div>
    ...
    <div class="form-group">
      ...
      <div>
        <%
          User.Gender gender = targetUser.getGender();
          String maleChecked = (gender == User.Gender.MALE) ? "checked" : "";
          String femaleChecked = (gender == User.Gender.FEMALE) ? "checked" : "";
          String otherChecked = (gender == User.Gender.OTHER) ? "checked" : "";
        %>
        ...
        <%
          for (User.Type type : User.Type.values()) {
            ...
            if (type == User.Type.ADMIN) {
              displayText = "管理员";
            } else if (type == User.Type.READER) {
              displayText = "普通用户";
            }
            String selected = (targetUser.getType() == type) ? "selected" : "";
        %>
        <option value="<%= type.name() %>" <%= selected %>><%= displayText %></option>
        <%
          }
        %>
    ...
    <div class="form-group">
      ...
      <textarea class="form-control" id="comment" name="comment" rows="4"><%= targetUser.getComment() %></textarea>
    </div>
